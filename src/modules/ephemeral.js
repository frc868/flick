    ...
    init: function({ config, db, serverId }) {
        return {
            onReady: async function({ dclient }) {
                ...
                while (true) {
                    ...
                    for (let msg of messages) {
                        ...
                        setTimeout(() => {
                            debug(
                                `deleting in #${msg.channel.name} (${msg.guild.name}) (leftover)`
                            );
                            msg.delete();
            ...
            onMessage: function({ msg }) {
                ...
                debug(
                    `deleting ${msg.id} in ${msg.channel.name} (${msg.guild.name}) in ${config.expiry}s`
                );
                setTimeout(async () => {
                    debug(
                        `deleting in #${msg.channel.name} (${msg.guild.name}) (live)`
                    );
                    await msg.delete();
                }, config.expiry * 1000);
            }
