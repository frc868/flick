...
module.exports = {
    ...
    init: ({ config }) => {
        const division = add => async ({ msg, rawArgs }) => {
            ...
            let roles = [];
            while (r < separatorRole.position) { // get all roles under seperator role
            ...
            if (add) {
                ...
                if (roles.map(x => msg.member.roles.has(x)).includes(true)) return "You're already in a division!";
                if (msg.member.roles.has(role.id)) return "You already were :)";
                if (role.comparePositionTo(separatorRole) >= 0) return "no";
                await msg.member.addRole(role);
                return "Added!";
            } else {
                const current = msg.member.roles.find(
                    x => roles.includes(x.id)
                );
