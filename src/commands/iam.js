...
module.exports = {
    ...
    init: ({ config }) => {
        const iam = add => async ({ msg, rawArgs }) => {
            ...
            if (add) {
                if (msg.member.roles.has(role.id)) return { title: "You already were!" };
                if (role.comparePositionTo(separatorRole) >= 0) return { title: "You buffoon. You fiend. You absolute cretin. Stop trying to escalate permissions." };
                await msg.member.addRole(role);
                return { title: "Added!" };
            } else {
                if (!msg.member.roles.has(role.id))
                    return { title: "You weren't but now you aren't even more." };
                if (role.comparePositionTo(separatorRole) >= 0) return { title: "Yes you are." };
